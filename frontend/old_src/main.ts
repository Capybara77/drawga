...
const confirmWrapper = document.querySelector('.confirm-wrapper') as HTMLDivElement;
...
const settingsButton = document.querySelector('.settings-show-btn') as HTMLButtonElement;
...
const changeThemeBtn = document.getElementById('change-theme-btn') as HTMLButtonElement;
const themeText = document.getElementById('theme-inner-text') as HTMLDivElement;

const moonIcon = document.getElementById('moon-icon-container') as HTMLDivElement;
  ...
  try {
    ...
    switch (command) {
      ...
      case 'disconnect': {
        ...
        const list = document.getElementsByClassName('trailer');
      ...
      case 'drawObj': {
        const o = getTypedDrawObject(data[1], roughCanvas, ctx) as BaseObject;

        if (o.typeName === 'text') {
          const textObj = o as TextObject;
          const oldInputs = document.querySelectorAll('#' + textObj.inputId);
...
window.addEventListener('pointerup', (event) => {
  ...
  const messageToServer: string =
    'drawObj:::' + JSON.stringify(allObjects[allObjects.length - 1]) + ':::';
...
window.addEventListener('resize', () => {
  const strokeStyleTemp = ctx.strokeStyle;
  const fillStyleTemp = ctx.fillStyle;
  const lineWidth = ctx.lineWidth;
...
window.addEventListener('wheel', (event: WheelEvent) => {
  ...
  const screenCentreX: number = event.clientX;
  const screenCentreY: number = event.clientY;

  const normalOffsetX: number = (offsetXCustom + (currentZoom - 1) * screenCentreX) / currentZoom;
  const normalOffsetY: number = (offsetYCustom + (currentZoom - 1) * screenCentreY) / currentZoom;
  ...
  if (event.deltaY < 0) {
    offsetXCustom = normalOffsetX + (currentZoom - 1) * (normalOffsetX - screenCentreX);
    offsetYCustom = normalOffsetY + (currentZoom - 1) * (normalOffsetY - screenCentreY);
  } else {
    offsetXCustom = normalOffsetX + (currentZoom - 1) * (normalOffsetX - screenCentreX);
    offsetYCustom = normalOffsetY + (currentZoom - 1) * (normalOffsetY - screenCentreY);
  }
...
document.addEventListener('keydown', (event) => {
  ...
  if (isCtrl && codeValue === 'KeyZ') {
    if (event.shiftKey) {
      ...
      const lastBufferElement = bufferObj.pop() as BaseObject;
      allObjects.push(lastBufferElement);
      lastBufferElement.draw(offsetXCustom, offsetYCustom);
      const messageToServer: string = 'drawObj:::' + JSON.stringify(lastBufferElement) + ':::';
      socket.send(messageToServer.length as unknown as string);
      socket.send(messageToServer);
      return;
    }

    const userObjects: BaseObject[] = allObjects.filter((object) => object.userId === myId);

    if (userObjects.length === 0) {
      return;
    }

    const lastUserObject = userObjects[userObjects.length - 1];
    allObjects = allObjects.filter((item) => item !== lastUserObject);

    const a = JSON.stringify(lastUserObject);
...
const confirmAgreeBtn = document.getElementById('confirm-agree') as HTMLButtonElement;
const confirmCanselBtn = document.getElementById('confirm-cancel') as HTMLButtonElement;
...
const [colorPickerButton, colorListContainer, colorsItemList, colorPickerInput] =
  createColorElements('.color-picker', 'color-list', '#color-item', 'color-picker-input');
...
const [textColorPickerButton, textColorListContainer, textColorsItemList, textColorPickerInput] =
  createColorElements(
    '#text-color-picker',
    'text-color-list',
    '#text-color-item',
...
document.documentElement.addEventListener('click', (event) => {
  if (textColorListContainer.style.display === 'grid' && event.target !== textColorListContainer) {
    textColorListContainer.style.display = 'none';
  }
});
...
document.documentElement.addEventListener('click', (event) => {
  if (colorListContainer.style.display === 'grid' && event.target !== colorListContainer) {
    colorListContainer.style.display = 'none';
  }
});
...
const decreaseZoomBtn = document.getElementById('zoom-decrease') as HTMLButtonElement;
const increaseZoomBtn = document.getElementById('zoom-increase') as HTMLButtonElement;
...
function downloadFile() {
  const data = canvasElement.toDataURL('imag/png');
...
export function resizeTextEvent(event: Event) {
  ...
  const messageToServer: string = 'drawObj:::' + JSON.stringify(foundObj) + ':::';
