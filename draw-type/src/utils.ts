...
export function getTypedDrawObject(
  ...
  switch (type) {
    ...
    case "line": {
      let { color, width, startPoint, endPoint, userId, zoom } = json as LineObject;
    ...
    case "curve": {
      let { pointsList, color, width, userId, zoom } = json as CurveObject;
...
export function reDraw(
  ...
  for (let index = 0; index < objects.length; index++) {
    ...
    if (
      element.isOverlay(offsetXCustom, offsetYCustom, screenWidth, screenHeight)
    ) {
